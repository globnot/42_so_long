...
/*   Updated: 2026/02/01 14:19:06 by aborda           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	if (ac == 2)
	{
		...
		render_map(game);
		mlx_hook(game->win, DestroyNotify, ButtonPressMask, (int (*)())callback_cross, game);
		mlx_hook(game->win, KeyPress, KeyPressMask, (int (*)())callback_key, game);
		mlx_loop(game->mlx);
		free_map(map);
